...
<#if cardone.StringUtils.isNotBlank(parentId)>
    ...
    <#else>
    ${prefixName?string('WHERE ', 'AND ')}t.parent_id = :parentId
    </#if>
...
<@cardone.permissionSql prefixName=prefixName permission_departments=permission_departments  departmentColumnName="t.department_code" permission_users=permission_users userColumnName="t.personal_code" />