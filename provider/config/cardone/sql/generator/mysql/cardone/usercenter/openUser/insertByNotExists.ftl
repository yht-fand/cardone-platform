...
<#if (insert_openUserId??) && (insert_openUserId_value??)>
${prefixName} :insert_openUserId_value
<#assign prefixName = ','>
</#if>
<#if (insert_orgCode??) && (insert_orgCode_value??)>
...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM c1_open_user E
...
${prefixName} E.OPEN_USER_ID = :where_and_eq_openUserId_value
<#else>
${prefixName} E.OPEN_USER_ID IS NULL
</#if>
<#assign prefixName = 'AND'>
</#if>
